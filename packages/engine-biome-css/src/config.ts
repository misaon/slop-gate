...
/**
 * `enabledRuleIds` carries the elected selection forward so `run` can check every finding against it.
 * Biome's JSON report has no `number_of_rules`, so unlike `engine-oxlint` there is no count to compare
 * — the selection is the only thing an unelected finding can be caught against, and
 * `EngineConfigHandle` has nowhere to put it.
 */
...
export async function materializeBiomeCssConfig(
  ...
  // The Biome CSS rules this adapter elects take no options (`rules.ts` declares none), so the option
  // half of a setting is dropped and `rulesetHash` below need not fold it in.
  ...
  const enabled = [...selection]
    .filter(([engineRuleId, [level]]) => {
      ...
      // Both synthetic, so neither is a rule Biome's config format knows: writing either into
      // `linter.rules` makes Biome reject the whole configuration.
  ...
  const config = {
    ...
    // unelected rules report and bypass arbitration entirely. `parseBiomeOutput` is the second guard.
    linter: { enabled: true, rules: { recommended: false, ...rules } },
    // Nothing but linting. `oxfmt` owns formatting (spec §13.1) and Biome's assist actions would be a
    // second, unregistered source of findings.
    formatter: { enabled: false },
    assist: { enabled: false },
    css: {
      linter: { enabled: true },
      // **Both keys, always.** With `tailwindDirectives` on and `cssModules` left unset, Biome 2.5.6
      // silently stops recognising `.module.css` as a CSS Modules file and `:global(...)` becomes an
      // unknown pseudo-class — **265 false findings across 36 files** on the measurement corpus, all
      // artefacts of the pairing rather than of either key alone. Setting `cssModules` restores it.
  ...
  return {
    ...
    // `foreign-suppression` and `css-parse-error` reports were elected, and neither is in the config
    // file above because Biome would reject them.
 ...
 *
 * The directory holding `--config-path` becomes Biome's project root and **is scanned**. A `biome.json`
 * anywhere beneath it aborts the entire run — no findings, no partial result, exit 1, and an error text
 * about a file the user never wrote. Observed directly. The hash-named directory makes that impossible
 * in practice; this check turns a future change that reuses a shared `tmpDir` into an explicit failure
 * here instead of a baffling one from Biome.
 */
