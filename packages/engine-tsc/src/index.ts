...
async function* execute(
  ...
  const args = [
    ...
    project,
    '--noEmit',
    // `rootDir` is an emit setting and nothing here emits, but tsc enforces it while building the
    // program: a project with tests outside `rootDir` got one TS6059 per file and no type checking.
    '--rootDir',
    context.rootDir,
    '--pretty',
    'false',
