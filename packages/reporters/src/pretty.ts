...
import { brandHeader, createFrameKit, plural, type FrameKit } from './box.ts'
...
/**
 * What the summary and timing renderers need, and nothing else. They are the two longest pieces of this
 * file and neither touches the reporter's pending-diagnostic state, so they sit at module level and take
 * this instead of closing over a factory scope they only read four strings from.
 */
type Marks = {
  readonly checkMark: string
  readonly gapMark: string
  readonly multiplySign: string
  readonly statsSeparator: string
}

type Surface = FrameKit & Marks

const marksFor = (unicode: boolean): Marks =>
  unicode
    ? { checkMark: '✓', gapMark: '▲', multiplySign: '×', statsSeparator: ' · ' }
    : { checkMark: 'OK', gapMark: '!', multiplySign: 'x', statsSeparator: ' | ' }
...
export function createPrettyReporter(context: ReporterContext): Reporter {
  ...
  const { width, paint, writeUnit } = kit
  const surface: Surface = { ...kit, ...marksFor(unicode) }
  ...
  return {
    onEvent(event: CheckEvent) {
      if (event.type === 'diagnostic') {
        ...
        flushPending()
        writeSummary(surface, event.result)
      }
    },
  }
}
...
function packCacheCells(surface: Surface, cells: readonly string[]): string[] {
  const label = 'cache  '
  const continuation = ' '.repeat(displayWidth(label))
  const budget = Math.max(1, surface.width - 4 - displayWidth(label))
  const packed: string[] = []
  let current = ''
  for (const cell of cells) {
    const candidate = current === '' ? cell : `${current}${surface.statsSeparator}${cell}`
...
function writeSummary(surface: Surface, result: CheckResult): void {
  for (const failure of result.engineFailures) {
    surface.writeUnit([`  ${surface.paint(['bgRed', 'white'], ' ENGINE FAILED ')} ${failure.engine}: ${failure.message}`])
  }

  const gaps = result.unavailableEngines.filter((engine) => engine.displaced.length > 0)
  for (const gap of gaps) {
    surface.writeUnit([
      `  ${surface.paint(['bgYellow', 'black'], ' COVERAGE GAP ')} ${gap.engine} could not run here — ${gap.reason}`,
      `    ${surface.paint('yellow', `${plural(gap.displaced.length, 'concept')} went unchecked or to a lower-ranked rule.`)}` +
        (gap.install === undefined ? '' : ` ${surface.paint('yellow', `Resolve it with \`${gap.install}\`.`)}`),
    ])
  }
  ...
  if (total === 0) {
    ...
    lines.push(
      caveats.length === 0
        ...
        : `  ${surface.paint('yellow', `${surface.gapMark}  No issues found, but ${caveats.join(' and ')}`)}`,
    )
  } else {
    const parts = SEVERITY_ORDER.filter((severity) => result.counts[severity] > 0).map((severity) =>
      surface.paint(SEVERITY_STYLE[severity], plural(result.counts[severity], SEVERITY_NOUN[severity])),
    )
  ...
  if (cacheByEngine.some((engine) => engine.filesFromCache > filesFromCache)) {
    ...
    for (const line of packCacheCells(surface, ranked.map((engine) => `${engine.engine} ${engine.filesFromCache}/${engine.filesAssigned}`))) {
      lines.push(`  ${surface.paint('dim', line)}`)
    }
  }
  ...
  if (total >= MOST_FREQUENT_THRESHOLD) {
    ...
    for (const [concept, count] of top) lines.push(`    ${padStartDisplay(String(count), 3)} ${surface.multiplySign}  ${concept}`)
  }
  ...
  if (result.baseline !== null) {
    ...
    lines.push(
      accepted === 0
        ? `  ${surface.paint('dim', `${path} holds ${plural(entries, 'finding')}, none found here`)}`
        : `  ${surface.paint('yellow', `${path} accepted ${plural(accepted, 'finding')}, not counted above`)}`,
    )
    if (stale.length > 0) {
      lines.push(
        `  ${surface.paint('dim', `${plural(stale.length, 'accepted finding')} are fixed — run \`sgate baseline update\``)}`,
      )
    }
  }
  if (result.ruleset.overlaps > 0) {
    lines.push(
      `  ${surface.paint('dim', `${plural(result.ruleset.overlaps, 'rule overlap')} resolved — run \`sgate rules conflicts\` for detail.`)}`,
    )
  }
  if (result.ruleset.uncovered.length > 0) {
    const count = result.ruleset.uncovered.length
    lines.push(
      `  ${surface.paint('yellow', `${plural(count, 'enabled concept')} ${count === 1 ? 'has' : 'have'} no capable engine in this repo.`)}`,
    )
  }

  surface.writeUnit([surface.frameTop(), ...lines.map((line) => surface.frameRow(line)), surface.frameBottom()])
  writeTimings(surface, result)
}

function writeTimings(surface: Surface, result: CheckResult): void {
  ...
  const lines = [`  ${surface.paint('bold', 'timing')}${surface.statsSeparator}${total} ms total`]
  ...
  for (const note of [
    ...
    for (const line of wrapText(note, Math.max(1, surface.width - 2))) lines.push(`    ${surface.paint('dim', line)}`)
  }
  surface.writeUnit(lines)
  ...
  const ruleLines = [`  ${surface.paint('bold', 'findings by rule')}${surface.statsSeparator}${surface.paint('dim', 'a count: no engine reports per-rule time')}`]
  const keyWidth = Math.max(1, surface.width - 8)
  for (const rule of shown) {
    ruleLines.push(`    ${padStartDisplay(String(rule.findings), 4)}  ${truncateStart(rule.ruleRefKey, keyWidth)}`)
  }
  if (omitted > 0) ruleLines.push(`    ${surface.paint('dim', plural(omitted, 'more rule'))}`)
  surface.writeUnit(ruleLines)
}
