...
export function createHadolintEngine(options: { binaryPath?: string } = {}): Engine {
  ...
  return {
    ...
    async materializeConfig(selection: EngineRuleSelection, context: RunContext) {
      ...
      // hadolint keeps some rules off whatever the threshold, so a selected one has to be named in
      // `override` to report at all. Findings are filtered against `selections` and re-severitied below,
      // which is why every selected rule can go in one bucket.
      const forced = enabled.map(([rule]) => rule).sort(compareStrings)
      await writeFile(
        path,
        [
          '# Generated by slop-gate. Do not edit; it is deleted when the run ends.',
          'failure-threshold: none',
          ...(forced.length === 0 ? [] : ['override:', '  error:', ...forced.map((rule) => `    - ${rule}`)]),
          '',
        ].join('\n'),
        'utf8',
      )
