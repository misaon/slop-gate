...
export type InstallHadolintOptions = {
  ...
  /**
   * Injected in tests so the success path can use locally generated bytes. The *mismatch* path is
   * exercised against the real `HADOLINT_CHECKSUMS` — a suite that only ever supplied its own digests
...
/**
 * D3, implemented directly: download the pinned release, verify it against the digest upstream published,
 * and only then put it somewhere that will be executed. **Nothing on the check path ever calls this** —
 * the only caller is `sgate engines install`; see `createActionlintEngine` and spec §13.5.
 *
 * **Simpler than the actionlint equivalent by one whole moving part.** hadolint publishes the executable
 * directly rather than inside an archive, so there is no tar reader here and no `.zip` to be blocked on —
 * which is why this adapter supports Windows x86_64 where actionlint's does not.
 *
 * **The digest is checked before anything is written, not after.** The bytes are hashed in memory and a
 * mismatch throws having created nothing; the verified binary then lands via a temporary file and a
...
export async function installHadolint(options: InstallHadolintOptions = {}): Promise<InstallHadolintResult> {
  ...
  if (expected === undefined) {
    // Unreachable while `HADOLINT_ASSETS` and `HADOLINT_CHECKSUMS` are transcribed from the same upstream
    // file — but an unverified download is what this module exists to refuse, so it refuses rather than
    // trusting that invariant at run time.
