...
import type { RuleLevel, RunContext } from '@misaon/slop-gate-core'
...
test('emits one document per declared language, not one per rule', () => {
  ...
  const config = buildAstGrepConfig(new Map([['slop-swallowed-error', 'warn']]))
...
test('reuses one rule id across every language document', () => {
  ...
  const config = buildAstGrepConfig(new Map([['slop-narrative-comment', 'warn']]))
...
test('writes the elected level in ast-grep spelling', () => {
  expect(buildAstGrepConfig(new Map([['slop-double-cast', 'warn']])).text).toContain('severity: warning')
  expect(buildAstGrepConfig(new Map([['slop-double-cast', 'error']])).text).toContain('severity: error')
  expect(buildAstGrepConfig(new Map([['slop-double-cast', 'info']])).text).toContain('severity: info')
})
...
test('throws rather than silently dropping an elected id this package has no rule for', () => {
  ...
  expect(() => buildAstGrepConfig(new Map([['slop-invented-rule', 'warn']]))).toThrow(/slop-invented-rule/)
})
...
test('writes the rule file and removes it on dispose', async () => {
  const handle = await materializeAstGrepConfig(new Map([['slop-double-cast', 'warn']]), context)
...
test('escapes a message or note containing an apostrophe', () => {
  ...
  const config = buildAstGrepConfig(new Map(ASTGREP_RULES.map((rule) => [rule.engineRuleId, 'warn' as RuleLevel])))
