...
const ids = (edits: readonly CandidateEdit[]): string[] => edits.map((e) => e.ruleId)

test('disjoint edits are all applied, sorted by start offset', () => {
  const later = edit({ range: { start: 20, end: 25 }, ruleId: 'oxlint/b' })
  const earlier = edit({ range: { start: 5, end: 8 }, ruleId: 'oxlint/a' })
...
test('exactly adjacent ranges do not overlap and are both applied', () => {
  const first = edit({ range: { start: 4, end: 10 }, ruleId: 'oxlint/a' })
  const second = edit({ range: { start: 10, end: 14 }, ruleId: 'oxlint/b' })
...
test('a fully nested range loses to the enclosing one when the enclosing one has higher priority', () => {
  const outer = edit({ range: { start: 4, end: 30 }, ruleId: 'oxlint/outer', priority: 90 })
  const inner = edit({ range: { start: 10, end: 14 }, ruleId: 'oxlint/inner', priority: 10 })

  const result = arbitrateEdits([inner, outer], 100)

  expect(ids(result.applied)).toEqual(['oxlint/outer'])
  expect(result.dropped).toHaveLength(1)
  expect(result.dropped[0]?.edit.ruleId).toBe('oxlint/inner')
  expect(result.dropped[0]?.reason).toBe('overlap')
  expect(result.dropped[0]?.winner?.ruleId).toBe('oxlint/outer')
})

test('a fully nested range wins over the enclosing one when it has higher priority', () => {
  const outer = edit({ range: { start: 4, end: 30 }, ruleId: 'oxlint/outer', priority: 10 })
  const inner = edit({ range: { start: 10, end: 14 }, ruleId: 'oxlint/inner', priority: 90 })

  const result = arbitrateEdits([outer, inner], 100)

  expect(ids(result.applied)).toEqual(['oxlint/inner'])
  expect(result.dropped[0]?.edit.ruleId).toBe('oxlint/outer')
})

test('partially overlapping ranges: registry priority decides', () => {
  const low = edit({ range: { start: 4, end: 12 }, ruleId: 'oxlint/low', priority: 10 })
  const high = edit({ range: { start: 8, end: 20 }, ruleId: 'oxlint/high', priority: 60 })

  expect(ids(arbitrateEdits([low, high], 100).applied)).toEqual(['oxlint/high'])
})

test('equal priority falls through to severity', () => {
  const warn = edit({ range: { start: 4, end: 12 }, ruleId: 'oxlint/a-warn', severity: 'warn' })
  const error = edit({ range: { start: 8, end: 20 }, ruleId: 'oxlint/z-error', severity: 'error' })
...
test('equal priority and severity falls through to rule id, ascending', () => {
  const first = edit({ range: { start: 4, end: 12 }, ruleId: 'oxlint/aaa' })
  const second = edit({ range: { start: 8, end: 20 }, ruleId: 'oxlint/bbb' })

  expect(ids(arbitrateEdits([second, first], 100).applied)).toEqual(['oxlint/aaa'])
})

test('the result does not depend on input order', () => {
  const a = edit({ range: { start: 0, end: 10 }, ruleId: 'oxlint/a', priority: 30 })
  const b = edit({ range: { start: 5, end: 15 }, ruleId: 'oxlint/b', priority: 70 })
  const c = edit({ range: { start: 12, end: 20 }, ruleId: 'oxlint/c', priority: 50 })
...
test('a chain drops only the middle edit when the two ends do not touch each other', () => {
  ...
  const a = edit({ range: { start: 0, end: 10 }, ruleId: 'oxlint/a', priority: 90 })
  const b = edit({ range: { start: 8, end: 22 }, ruleId: 'oxlint/b', priority: 10 })
  const c = edit({ range: { start: 20, end: 30 }, ruleId: 'oxlint/c', priority: 80 })

  const result = arbitrateEdits([a, b, c], 100)

  expect(ids(result.applied)).toEqual(['oxlint/a', 'oxlint/c'])
  expect(result.dropped.map((d) => d.edit.ruleId)).toEqual(['oxlint/b'])
})

test('two zero-width insertions at the same offset conflict', () => {
  const a = edit({ range: { start: 7, end: 7 }, replacement: ';', ruleId: 'oxlint/a', priority: 90 })
  const b = edit({ range: { start: 7, end: 7 }, replacement: ',', ruleId: 'oxlint/b', priority: 10 })

  const result = arbitrateEdits([a, b], 100)

  expect(ids(result.applied)).toEqual(['oxlint/a'])
  expect(result.dropped[0]?.edit.ruleId).toBe('oxlint/b')
})

test('a zero-width insertion at the start of a replacement conflicts with it', () => {
  const insert = edit({ range: { start: 7, end: 7 }, replacement: '_', ruleId: 'oxlint/insert', priority: 10 })
  const replace = edit({ range: { start: 7, end: 12 }, replacement: 'name', ruleId: 'oxlint/replace', priority: 90 })
...
test('zero-width insertions at different offsets are both applied', () => {
  const a = edit({ range: { start: 7, end: 7 }, replacement: ';', ruleId: 'oxlint/a' })
  const b = edit({ range: { start: 9, end: 9 }, replacement: ';', ruleId: 'oxlint/b' })
...
test('an edit reaching past the end of the buffer is dropped, never clamped', () => {
  const beyond = edit({ range: { start: 90, end: 120 }, ruleId: 'oxlint/beyond' })
...
test('an inverted range is dropped as out of range', () => {
  const inverted = edit({ range: { start: 20, end: 10 }, ruleId: 'oxlint/inverted' })
...
test('an out-of-range edit does not consume the overlap slot a valid edit needs', () => {
  const broken = edit({ range: { start: 0, end: 500 }, ruleId: 'oxlint/broken', priority: 99 })
  const good = edit({ range: { start: 4, end: 8 }, ruleId: 'oxlint/good', priority: 1 })
