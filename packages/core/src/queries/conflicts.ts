...
export type RulesConflicts = {
  /** A concept with more than one loser appears here more than once, by design: a reader wants the reason for
   *  *each* losing rule, not a pre-grouped summary that hides how many there were. */
...
/**
 * Almost a pure formatter: every field here is already sitting on the resolved run, computed once by config
