...
test('no generated concept duplicates a written one', () => {
  ...
  // A floor, not a target, and it falls as concepts are written up: every rule `recommended` enables
  // carries a curated description, which has moved 300 of these into `curated.ts` so far. Lower it when
  // that continues; a jump the other way means the generator stopped seeing the written ones.
  expect(GENERATED_CONCEPTS.length).toBeGreaterThan(200)
})
