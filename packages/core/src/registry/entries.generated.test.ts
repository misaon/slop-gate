...
test('no generated concept duplicates a written one', () => {
  ...
  // A floor, not a target. Every concept `recommended` enables now carries a curated description, so
  // what is left generated is the unlisted and withheld tail. A jump upward means the generator stopped
  // seeing the written ones.
  expect(GENERATED_CONCEPTS.length).toBeGreaterThan(50)
})
