...
**`--max-warnings 0`, not bare `sgate check`.** Only 239 of the 636 concepts `recommended` enables here
are `error`; 396 are `warn`, including all four `slop.*` rules. Measured: a file with an `as any` cast
